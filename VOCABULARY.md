...
| **the top bar** | the header line, and it always answers **"where am I"** — most specific wins: the **estate** you are in > the **person** signed in > **"My Home Place"**, the product itself (the STARTING name `[paul-ruled 2026-09-11 ~2:55 AM ET: "Let's just go with my home place for now and bold home in between my and place… that'll make everything coherent when we move to my home dot place"]` — wordmark My **Home** Place, said "my home dot place"; a person's own name for their place wins the moment they give one; *Estate Manager* is the administrator's back end only, never a user-facing name). Never decoration | `paul-stated 2026-09-05` |
...
4. ✅✅ **RULED 2026-09-12 — THE PRODUCTION DEPLOYMENT IS A NEW ONE: `myhome-prod`** `[paul-stated:
   "myhome-prod is good"]`. **Neither standing deployment survives as production.** `fernwood-home`
   (`est-e6696a`, Mom) and `myhome-paul` (`est-d93508`, Paul) BOTH migrate into it as rows, and both
   retire once their migration is verified. `myhome.place` attaches to `myhome-prod`.
   ⭐ **Why a third thing rather than either candidate:** both existing names are household-specific — one
   carries a person's name, one an estate's — and production holds everyone. Pages project names are
   IMMUTABLE, so a name neither of those could only be reached by creating one. It costs a second
   migration and buys a front door nobody has to explain.

   ### ⛔⛔ AND THE CONSEQUENCE THAT CHANGES THE ORDER OF WORK, measured rather than assumed
   ...
   *which estate is this REQUEST for?* — has **4**. ⛔ **Migrate two households into one deployment before
   ...
   exactly `.plans/2026-09-11-lap8-build-PLAN.md` § ROW A · the door (A0–A15: the scope classifier, the
   B-CALLER conversion, `X-Estate`, the `grant:<personId>:<estateId>` edge, `estates[]`, the single
   sign-in page) — a build plan that is written, audited, and never executed. **The cutover and the door
   ...
   deployments (recommendation `myhome-paul`, and a 09-10 "yes to all" against that framing). The question
   was malformed; recorded as a supersession rather than replaced silently.~~
...
| ✅ **the Journal — RULED, and it fills the hole the row above left** | `[paul-ruled 2026-09-10]` — *"if journalName is what we're gonna call it, let's just call it the Journal."* The **portable noun** for an estate's record and the inquiry over it. It passes §4's own test where *Almanac* failed: it names an **object in a house**, promises no season and no genre, and is as true of a gardenless condo's receipts as of 178 month-keyed season notes. ⭐ It is also already the project's voice — CLAUDE.md has said *"a field journal, not a task manager"* since the start. ⚠️ **`journalName` per instance is unchanged and still governs**: the Journal is the engine word, *"Fernwood Almanac"* stays what Fernwood calls its own — "each estate names its own thing" was never overturned, it now has a word for the thing. ⛔ Retires **"Garden Guru" as the portable name** for the capability: Guru is Fernwood's assistant, the Journal is what every estate has. |
