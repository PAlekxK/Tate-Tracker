...
| **27** | 🧭 **HOW THE MENUS ALIGN — the jump strip · the summary tiles · the cards** `[paul-stated 2026-09-10]` ⚙️ engine. Paul: *"we should have a backlog item on kind of how the menus align between the jump strip, the summary menu, and the cards. I think that we've accumulated a lot of formatting questions and kind of structure questions, and we're overdue for UX review."* **He is right on both counts, measured:** `check-ux-sweep.py` reads **OWED — 121 commits to `viewer.html` against the 20 limit, last two-pass run 2026-08-31**; and the alignment question was already on the record as **C7-R1** (his 09-04 condo read: *"three layers repeat one thing (emoji · tile line · big card). Rationalize: which layer is the door, which the glance, which the room"*) — buried as a rider in a section set aside today, so it is lifted here. **The accumulated formatting and structure questions this row collects, by pointer, never restated:** C7-R1 (three layers, one thing) · C7-R2 (an OFF module must look intentional — the summary grid's gaps) · the seats' 09-10 reads on row 19 (three same-styled commit buttons on one card; 'Home Almanac' twice within 100px; Weather/Sky each three times on the first screen; the jump-strip chip truncating the name and dropping 'Almanac'; the 💬 bubble over type at A+) · the 08-09 *"Another question ›"* cycle legibility (A4 · W8·e) · the masthead utility bar aligned to nothing (TIER 2 · 21). **The instrument is the two-pass sweep, and it is already scheduled: TIER 1 · 26's first act is `/ux-sweep` on the candidate.** This row is what that sweep's pass 2 adjudicates the strip/tiles/cards question under; the design window brings Paul the options. ⛔ *Freshness sets altitude* and *one affirmative grammar* are the ruled principles the answer must satisfy; a single-seat review does not reset the sweep clock. | design · `viewer.html` (strip · tiles · cards) · `onboarding/index.html` · `homes/index.html` | *(refinement window, in its own voice, stamped as his; the sweep itself is row 26's)* |
...
- **C7-R1 · Jump strip vs summary tiles vs cards.** Three layers repeat one thing (emoji · tile line · big card). Rationalize: which layer is the door, which the glance, which the room. ux seat, then Paul. **→ LIFTED to TIER 1 · 27 on 2026-09-10** (C7 is set aside; this rider is engine-wide and survives it — the row is there, this line is its provenance).
