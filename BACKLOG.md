...
| **Populate from Paul's weed photos** | Paul intended to send weed photos on 2026-07-20 (**they did not attach to the message** — only the 13 fairway/meadow *grass* photos came through, which seeded the crabgrass entry). Each new weed gets an entry the same way: inferred ID + `needs-confirmation` + a draft Mom question. As Mom answers, fold `inferred→verified` and let what we learn **seed more entries + more cards** (Paul's stated loop). | AWAITING Paul's weed photos (re-send). |
