...
| **16** | 🔴 **GURU'S HARD-FACTS BLOCK INJECTS 2,959 ft — the stale elevation — and tells the model it overrides the digest** `[found 2026-09-03 by the ai-advisor seat; verified: worker.js carries "2,959" 14×, "2,873" 0×; canon has read 2,873 ft (USGS lidar, measured) since 08-31]`. The instrument built to stop the 2,800 ft error now injects an 86 ft one under a header that reads *"these override anything you infer from the digest below."* Live on her surface. **Fix is one prompt edit + a Worker deploy — but it is Mom-facing prompt content, so gated on Paul's read** (the standing Tier-1 note). ⭐ The durable fix is C5 step 5's config derivation: the hard-facts block reads `property.json`, never a typed number. | `worker/worker.js` § HARD FACTS | 🔴 **Paul's go, then deploy + `/health`** |
