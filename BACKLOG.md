...
| **Save / Ask two-button split** | Whether the unified input needs both buttons; likely hierarchy-not-removal (make Save primary, Ask quiet secondary). | Confirm with Paul what actually bugs him (clutter vs choice-friction vs one-intent-dominates) before touching it. Own thread — not part of the Mom queue. |
