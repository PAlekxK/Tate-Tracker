...
**Seats when scoped:** engineering-partner (the API path and what the geocoder already gives) · security-steward (roster). **Falsifier for the eventual build:** an address the geocoder placed but USPS rejects, or the reverse — if the two never disagree on real input, the second check bought nothing.
