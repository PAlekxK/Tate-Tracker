...
| **18** | 🟠 **`viewer.html` BOOTS TO A BLANK PAGE ON A PLANTLESS ESTATE** — six `getElementById` throw sites and eleven unguarded dereferences in `renderProperty` (measured by the C7 seat 2026-09-03); the first failure is a blank page, not a missing card. C7 step 0 (null-guards) is buildable now and pays at Fernwood regardless — this row exists so it is not lost if C7 stalls `[paul-approved 2026-09-03, C7 Q9]`. | `.plans/2026-09-03-c7-condo-paper-model-PLAN.md` step 0 | agent builds through C4's QA origin; `main` at Paul's gate |
