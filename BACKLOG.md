...
## A7 · Weeds — a new domain in the field journal (IDEATION, Paul 2026-07-20)
...
| Item | What it is | Gate / next |
|---|---|---|
| **Weeds section** | A weeds domain alongside plants/wildlife, layered like every rich domain: **glance** — a calm, timely "worth getting after this before it seeds" near-horizon read (field-journal tone, NOT urgency/alert language); **repository** — per-weed combat advice (how to fight it, when, organic-first) held one level down, not flooding the glance; **loop** — folded into the Mama's-Perspective ground-truth loop (Mom confirms which weeds are actually showing, and where). **Seed content:** Paul photographed the property's weeds on **2026-07-20** (in his photo library) to pull in when this is built — start the per-weed records from what's actually on the ground, not a generic weed list. **Authored content:** AI may draft the combat advice behind Paul's gate; Mom's surface + capture stay AI-free ([[feedback_no_ai_on_capture]]). | IDEATION — build NOT started (Paul: backlog it, 2026-07-20). Seed photos: Paul's library, 2026-07-20. |
| **"Before it seeds" timely nudge** | The weed-glance's concrete trigger — surface a weed at the moment it's about to set seed (the highest-leverage time to pull it). Seasonal/phenological like the bloom windows, but framed as a field-journal observation (*"the bittercress is about to seed — worth pulling now"*), never a deadline or alert. | Part of the weeds glance; after the section exists. |
