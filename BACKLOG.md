...
| **Transcribe Mom's voice captures → text (off-device, Paul-side)** ⭐ (Paul, 2026-07-18) | Convert the staged zone-audio `.webm` recordings into usable text files so what she says is **searchable + can feed the fold-to-canon step** (her spoken plant/zone knowledge → the record). Extends `read-mom-zone-audio.py`. **⚠️ AI-BOUNDARY (load-bearing):** transcription IS a model read of HER VOICE — so it must stay the *sanctioned off-device seat*: runs on **Paul's machine**, **POST-storage** (audio is stored verbatim + AI-free first), **Paul-facing** (a transcript NEVER reaches Mom's surface and NEVER auto-folds to canon), and the transcript is a **hypothesis until Paul checks it against the audio** (model-read-flags-never-clears — the exact reason on-device Web Speech was rejected: it mangles the nicknames we're mining for). | DEFERRED — build when Paul wants captures usable. Reuse candidates: OpenAI **Whisper** API (the Worker already has a dormant Phase-H OpenAI audio path; needs `OPENAI_API_KEY`) or a local whisper (whisper.cpp / mlx-whisper). *(Quick cross-project scan found no transcriber on PATH — locate/confirm at build.)* Low-risk (Paul-side, read-only). |
