...
| **14** | 🧬 **ONE DIGEST, N ESTATES — Fernwood's property record is in every household's Guru** `[found 2026-09-08 · scoped 2026-09-08]` ⚙️ engine · **must-not-diverge**. ⭐ **THE POSITIVE CONTROL FIRED — this is demonstrated, not inferred from the import statement.** `measured` 2026-09-08: one turn to `/api/chat` on **`est-qa0001`, an estate that is not Fernwood**, asked only *where is this property, what elevation, three plants, one vehicle* — and it answered **"282 Church Mountain Road, Jasper, GA 30143 (Pickens County)" · "2,873 feet… USGS 3DEP 1 m lidar" · White Pine / Mountain Laurel / Azalea · "2016 VW GTI (MK7 Autobahn, APR Stage 1 tuned)"**, then volunteered the Cherokee-to-Col.-Sam-Tate property story unprompted. **HOW FAR IT GOES: further than the digest.** `worker.js:68` imports the one 610 KB artifact statically (40 plants · **23 zones by name** · 23 vehicles · the whole property record), and `wrangler.toml` carries no per-env digest binding — but `FACTS` (`worker.js:288-312`) also **interpolates the street address, county, elevation, frost dates and reference station into the system-prompt TEXT**, so even a refusal-shaped answer carries the address. ⛔ **`check-estate-neutral` cannot see any of it** — it tests shipped pages for names; this is the model's prompt. ⭐⭐ **THE LIVE EXPOSURE IS AN UNSET SECRET, NOT A CONTROL** — `measured` from each Worker's own `/health.configured`: `myhome-bob` (est-9a74df) and `myhome-paul` (est-d93508) read **`anthropic: false`**, so `handleChat` 503s at `worker.js:2171` — the *same predicate* `/health` reports, not a proxy for it. **One `wrangler secret put ANTHROPIC_API_KEY --env bob` turns Bob's Guru into a reader of another family's street address**, with nothing in the product showing it. ⚠️ **`fernwood-home` (est-e6696a) reads `anthropic: true` — it is LIVE today.** There the harm is not privacy (it is Mom's own place) but **measurement**: the blank slate that exists to be the ANSWER KEY for a from-scratch build is not blank in the model's context. ⭐ **THE FIX ALREADY EXISTS IN THIS REPO, ON THE SIBLING ARTIFACT.** `build-viewer.py:276` resolves `cfg["canon"]` per instance and `:290-292` is hardened against **precisely this class** (an instance declaring domains absent while pointing `canon` at a populated directory, and shipping that household's entire canon anyway). The digest build never learned it: `compose()` (`build-digest.py:561`) **already takes an estate and already omits modules the estate does not declare** — but `main()` calls `momlib.estate()` (this checkout's = Fernwood's) and writes the one hardcoded `worker/digest.json` (`:625`). **Three things missing: a per-estate build target · a per-deploy binding · a stamp + refusal so a mismatch is LOUD** (the codebase's own idiom — `LEGACY_BEFORE` throws when absent). ⚠️ **AND IT IS NOT A 20-MINUTE JOB — `measured`, I ran it.** `compose()` against `instance/bob.json` → `instance/neutral-canon` reaches `FileNotFoundError: plants.json`, because that estate's `estate.json` declares **eight modules ON over a directory holding only `estate.json` + `property.json`**. The viewer has a concept for that (`absent`, and R5's *empty, not absent*); the digest has none — a declared-ON module with no file raises. **That is row 12's seam, and this is its FIFTH consumer and the only READ-side one** (12's four are all writes). ⛔ **Blocks the pending worker deploy by Paul's own call** — the frost-provenance correction is committed, the digest is fresh on disk and deliberately undeployed; *"it should ship with TIER 2 · 14 rather than alone."* **Falsifier:** a household Guru asked *where is this property* answers from that household's own record or says it holds none — **and** a Worker bound to an estate whose digest was built for a different estate refuses to start. → SCAN §13d/§13e · row 12 | 🔴 **Paul — deploy decision** |
