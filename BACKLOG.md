...
| **14** | 🧬 **ONE DIGEST, N ESTATES — Fernwood's property record is in every household's Guru** `[found 2026-09-08]` ⚙️ engine · **must-not-diverge**. `measured`: `worker/digest.json` is **one file, built from this repo's canon and bundled into whichever worker is deployed**; `wrangler.toml` carries **no per-env digest binding**. So the model context every estate's Garden Guru runs on is **Fernwood's** — its plants, its property record, its frost dates. ⭐ **This is the instance-content-inside-engine class (SCAN §2 / slate 2) one layer deeper: not on the screen, in the MODEL'S PROMPT**, where `check-estate-neutral` cannot see it at all. ⚠️ **Found while trying to ship a one-line frost-provenance correction** — the correction is committed and the digest is fresh on disk, **deliberately undeployed**, because the only estate the correction is *about* is the frozen Fernwood and that deploy is gated by the data-control ruling. ⛔ **Not investigated — scoped as a row rather than chased at 1am.** First question: does a household's Guru answer from Fernwood's canon today, and if so how far does it go? | ⛔ unscoped |
