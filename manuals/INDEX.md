...
| `dr200s-2017-service` | Suzuki **DR200SE Service Manual** (99500-…) | 262 | 🔴 **WRONG MODEL — it is a DR200**SE** manual, not DR200S** (falsified at the machine 2026-08-30, `paul-stated`: the manual says starter = 'Electric and kick'; **the DR200S has NO KICKSTARTER**). ~~DR200S unchanged across years~~ — that note answered a question about YEARS and was read as covering MODELS. Two more specs disagree with the vehicle card: stroke 58.2 vs 58.0 mm, compression 9.4 vs 9.5:1. Still the best doc we hold (shared 199 cc engine + BST31SS carb) but **every value taken from it is `inferred`, never `verified`** — see `dr200s-2017._manualProvenanceWarning`. OCR text is sparse (scan). | djebel-club.ru (enthusiast archive) |
