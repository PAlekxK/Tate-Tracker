...
- stage: design
- stage-note: 2026-09-10 close — the replace question is ANSWERED: **`zones.json` stays the raw trace; the cleaned set stays labelled as the leading candidate and is watched, not promoted** `[paul-stated 2026-09-10: "this is our leading zones, but we'll just keep it labeled and keep an eye on it until it's ready to kinda use it"]`. Zones window closed on his instruction. Lane state at close: candidate at `a5757b2` · Z-13 in §3 · ux-expert review filed · register edits landed at `d9ef14f` · owed and untouched: R-A1…R-A6 (not brought to him — the cleanup took the window), the driveway-buffer test (cleared, not run), `fetch-frame.py` (not started), content-steward drafting.
- stage-note: 2026-09-10 late — ⭐ **`.engineering/zones-derivability/zones.cleaned.json` IS THE LEADING CANDIDATE for Fernwood's zones** `[paul-stated 2026-09-10: "we can record this as the leading candidate for the zones now, but we're not gonna push it into the environment or anything until mom has created Fernwood and is ready"]`. The bank ↔ eastern-woodlands gap is closed by his "Yes" (17 m² to the woodlands). ⛔ `zones.json` is NOT replaced (he said *record*, not *replace*; asked, unanswered) and NOTHING reaches an origin until Mom has founded her Fernwood and is ready — his words, which also settles the preload's timing: after her founding, not after this lap's gate.
- stage-note: 2026-09-10 late — ux-expert reviewed the cleaned set AS THE PRELOAD → `.ux-reviews/2026-09-10-zones-cleaned-preload.md` (8 findings). Top three: FIT THE FRAME (`MIN_SCALE = 1`, the property fills ~15% of the stage and every zone renders as a dashed draft — the preload makes that the whole map) · the built core is a smear at 414 × A+ (glance/repository split: cluster at the glance, zoom on tap) · **do not ship 23 confirms** — derive `status` from USE and ask the one set question she already asks, *is anything missing*. The two geometry defaults are sub-perceptual at her zoom and do not gate the preload. Two candidate-file notes: the one surviving gap (bank ↔ eastern woodlands) reads as *something is missing here* — close it or name it; `house` is typed `structure` but renders in the planted grammar.
- stage-note: 2026-09-10 evening ET — **Z-13 ruled in the zones window** (preload-then-confirm; reverses J-f for zones, on purpose, for practicality — *"this is becoming a huge bottleneck"*). Cleanup of the 23 starts as a dev-only candidate in `.engineering/zones-derivability/`; `zones.json` untouched until his go.
...
| **Z-7** | ⚠️ **ITS OWN SITING REQUIREMENT IS UNMET — measured 2026-09-07.** The ruling says the re-open trigger must be *"written where the code will be"*; `git grep -i "selects or filters"` across `*.js *.py *.html` returns **ZERO**. So it is recorded in a register and binds no reader who never opens this file. ⭐ **The general rule it implies, and it is unwritten: a ruling that governs an ACT is sited AT THE ACT; a ruling that governs a DECISION is sited in the register.** `CLAUDE.md` already states the special case (*"pages-deploy CALLS check-estate-neutral and refuses on a hit — a check wired into the thing it guards cannot be forgotten"*); the generalisation is one sentence nobody has written. **Debt, not done.** — *"Do we just want to provide links to various helpful websites to start? I think we can scope that together."* + `[paul-ruled]` *"that's fine on J-e."* | Events/neighbourhood starts as **LINKS** — membership by rule, nothing filtering, **no AI-boundary ruling owed.** ⭐ Re-open trigger, written where the code will be: **the first time anything SELECTS or FILTERS what appears on a card.** |
...
| **Z-13** | ⭐⭐ **PRELOAD, THEN CONFIRM — Mom's zones are drawn OUTSIDE the app and she confirms them** `[paul-stated 2026-09-10, this window, five messages, verbatim]`. *"I think one thing I'd like to do is go ahead and take the 23 zones that we've got defined for Fernwood and try to clean them up visually a little bit so there's no gaps and they just kind of look better. More cohesive shapes not depending too much on just those vertices I clicked and that'll give us you know zones that we can start to learn from."* · *"that way we can kind of preload the zones into mom's estate when she sets it up in production and you have her confirm them rather than draw them as we continue to think through how do we create them more systematically"* · *"I don't wanna be overly complicated, but I do wanna say that we should make a note that you know for Mom we're planning to do is essentially skip the zone definition process and I'm just doing that all outside of the app, but the goal is down the road for that to be you know process we continue to develop, but I hope that this unlocks everything that's downstream of having zones so we can learn from that without figuring out the perfect way to find them kind of in the application so we've got the short-term and long-term vision"* · asked whether this reverses J-f: *"question one is a reversal just for practicality as I said, and to push our overall build along this is becoming a huge bottleneck"* · on the cleanup method: *"go ahead and clean it up as best you can automatically and nothing should overlap. I don't think but you can let me know of any questions"* | ⛔ **REVERSES J-f FOR ZONES, ON PURPOSE** (recorded as a noticed reversal per the 08-04 "everything is changeable" caveat): Mom's production estate starts with the cleaned Fernwood zones **pre-drawn as drafts she confirms**, not blank. **Short term:** the operator (Paul) defines zones outside the app; the cleaned 23 are the preload AND the answer key. **Long term:** the systematic definition process (Process B, §5, the assessment) continues as research — Z-9 unchanged, the manual step is still scaffolding. ⭐ **The point is to unblock everything DOWNSTREAM of having zones** (v1 zones × plants, the cascade Z-11) without first solving in-app definition. **Method ruled:** close gaps automatically, **no overlaps**, questions surfaced to him on the exhibit. ⚠️ **What it does NOT reverse:** Z-10 (the frozen PLANT record does not travel) · §8·0 · Bob gate 3 never gate 1 · the G1 line for the PRELOAD ACT itself (a write to `home`, sequenced by coordination after this lap's gate — *"never deploy to home, paul, bob or legacy"* while the founding walk runs at qa). The CLEANUP runs dev-only in `.engineering/zones-derivability/`, candidate file + before/after exhibit; `zones.json` is replaced only on his go, through the window that owns the write. |
...
- ~~⛔ **Mom starts BLANK** `[paul-ruled 2026-09-07, J-f]` — nothing pre-filled. The 23 hand-traced zones stay
  on the frozen instance as the **ANSWER KEY**. **Irreversible if broken.**~~ ⚠️ **REVERSED FOR ZONES by Z-13
  `[paul-stated 2026-09-10]`** — her map starts with the cleaned 23 pre-drawn as drafts to confirm. Still true
  for PLANTS (Z-10) and for every other domain. The cleaned set becomes the answer key; the raw trace stays in git.
...
| horizon | what | plan |
|---|---|---|
| ⛔ **LEG 0 — before either** | **the per-estate capture write path.** ⚠️ Smaller than first framed: five capture handlers already work at `home`; exactly one create is broken (`handleZoneSave`'s 503 gates a KV write it should not). ⛔ **R-Z6(B) is a same-commit co-requisite** | `.plans/2026-09-07-capture-write-path-PLAN.md` · `handoff/handoff-capture-write-path.md` · BACKLOG ▶️ NEXT · TIER 2 · 8 |
| **V1 — now** | ⭐ **ZONES FIRST** `[Z-10]` — she and Paul define places on a blank instance; **then** plants are added through the photo → confirm path, carrying their place at the moment of adding. Species-level, names lead. ⚠️ **An AMENDMENT to a shipped surface** (§0 ①), not a build | **this file** · `.user-research/2026-09-07-zones-plants-v1-journey.md` (REV 3, the first walkthrough) |
| **NEAR** | the three primitives in the record (RFC 7946 nested at `zone.geometry`) · a **plural, typed** place field on the nine domains that have none · household-system **points**, which need the missing *subjects* first (well · septic · shut-off · spigots) | ⬜ **declared, deliberately no plan file** — raised to be findable, not to be scheduled |
| **LONG** | the derived first draft from an address · geometry leaving git · a map surface built into the neutral journey (⚠️ **there is no map in production at all**, and the deploy allow-list enforces it) · the illustrated map · project sites over time | `.plans/2026-09-07-derived-first-draft-PLAN.md` (Process B) · `.plans/2026-09-07-mapping-sources-SCAN.md` · `.plans/2026-09-06-ai-mapping-capability-SCAN.md` |
| **STANDING RESEARCH** `[Z-8]` | ⛔ **two hypotheses are DEAD and recorded as such** — gradient-magnitude confidence (livewire adheres to the strongest edge, which on a 33°-sun January frame *is* the shadow) and multi-frame shadow consensus (NAIP shadows do not move, they shorten, so the intersection of seven certifies the shortest one as real). ⭐ **Live:** the mowing-regime time-series test · the lidar slope toggle · what the answer key can measure | ⬜ lives in `## Falsifier`, per §2c of the tracking design |

⛔ **NO STATE IS TYPED IN THIS TABLE, and that is a repair.** It carried a `state` column until 2026-09-07 late,
and **it was already stale within hours of being written** — it read *"Not started"* for LEG 0, which by then
had a plan, a BACKLOG row, a handoff and two pre-registered checks, and *"has not begun"* for a LONG row that
had acquired `derived-first-draft-PLAN.md`. **Each plan's own `stage:` is the state**
(`grep -H '^- stage:' .plans/2026-09-07-*-PLAN.md`), exactly as `BACKLOG.md` § FOCUS FREEZE already rules for
the C-series. ⭐ Found by practice-steward within hours of my writing it — *"a count typed beside a tool that
computes the same count"* is this corpus's most repeated defect and I had just committed a fresh instance.
