...
- stage-note: 1b/1c BUILT ON QA 2026-09-03 10:31 PM ET (commit "C6 1b/1c: the served text-size default is instance config" — first pushed as 5d7760f; the sha moves under rebase, the subject is the key `qa-divergence.py` matches) — landed as instance config directly (no interim literal ever existed, so 1b's lint-out step was moot): `instance/fernwood.json display.defaultTextSize: "lg"`, `{{DISPLAY:defaultTextSize}}` in the template, the decision block amended in place with the 9/03 evidence; the condo fixture declares `normal` and its build carries it (per instance, no fork). Checks: `build-viewer.py --check` 0 · `grep -c '{{'` = 0 · both selftests green incl. the planted `DEFAULT_SIZE = "lg"` under engine/ FIRING · `tools/check-text-size-default.py` on the built file AND on QA: fresh {lg, stored:false}, no key written, stored-normal renders A; its negative control (`--expect normal`) exits 1 · `qa-walk` clean at 414×A+ · `check-live --ref origin/staging` all green · condo falsifier HOLDS. **main NOT pushed** — the served default changes what an unconfigured device shows, so it rides the migration [paul-stated 2026-09-03: build in QA, Paul manages the migration with Mom].
- stage-note: privacy F9 / seat I1 APPLIED 2026-09-03 11:05 PM ET (760073c, on QA): `declarePerson` is a GUARD — a record arriving with a non-null personId THROWS — and `attributeTo(record, grant)` is the only non-null writer (no caller until 4a/5). Done inside the window: no non-null person has ever been written. Proven in node (smuggled person throws; grant attribution sets personSource "grant"; a grant without estateId throws); QA `/health` ok and `/api/door` still stores after the deploy. Prod-eligible (engine, identical for Mom) but HELD with the migration: prod has no writer that could earn a person yet, and a prod push now would need the `prod` cherry-pick branch (BACKLOG, build-in-QA rule).
- stage-note: 3a BUILT 2026-09-04 12:45 AM ET — `tools/grant-mint.py` (mint · revoke · init-schema · --selftest 18/18, every KV call dry-run): the 3a row (`entry` · `vault` · `credential{hash,issuedAt,issuedBy,revokedAt}` · `consent[]` per Q2's full field set) declared on both Fernwood rows in the private register (sibling commit); G1 (a founding owner grant needs the owner's OWN `founding-request`; a relay is refused) and G2 (a non-administrator grant at an estate where the administrator holds no relationship needs `administrator-reads`) enforced AT THE MINT per Q3, no watcher; `access` cannot be hand-written; the token leaves once into a mode-600 file, the register holds the hash. Proven on QA with the real path: two fixture grants minted from `.private/grants-qa-fixtures.json` into QA KV (`est-qa0001` entry+vault founding owner; `est-qa0002` other-estate), `/api/grant/whoami` → 200 with the row for the first, 404 for the other; the two hand-minted rows retired. ⚠️ A request with no User-Agent gets Cloudflare's 403 before the Worker sees it — send one when verifying by use. Paul's real credential (4a/4b) is minted with the same tool; nothing here mints one.
- stage-note: seat F3 CLOSED 2026-09-04 1:15 AM ET [paul-stated "go for it now. No one is looking at it"]: AMBIENT_MAC left the public wrangler.toml on both envs and is a Worker secret (`wrangler secret bulk`, installed AFTER the var was removed and deployed — the API refuses a secret whose name is still a var, so the outage window was the deploy-to-secret gap, ~15 s each). Prod deployed from a `prod` branch cut from origin/main carrying only the toml change (the held viewer changes did not ride), commit 0d166b1 pushed to main; verified by use: prod `/api/ambient` 288 rows, QA rows. **Also:** a local `.git/hooks/pre-push` now runs `check-public-build.py` before any push [paul-stated "5 — go for it"]; mutation-tested (a planted needle blocked a dry push).
- stage-note: [paul-stated 2026-09-04 ~1:25 AM ET, on the vault/QA-instance question] *"this is where we start to potentially diverge from the live Fernwood mom sees? Let's do everything else that we can do before adding new features. But yes, then we need to make additions in a way that's trackable."* → 5a/5b and every other NEW SURFACE wait; non-feature work (instrumentation, migration prep, the divergence tracker that makes additions trackable) goes first.
- stage-note: 5b's "QA declares a fixture room" — PATH-EVALUATED 2026-09-04 (`.engineering/2026-09-04-qa-instance-overlay.md`, engineering-partner, at Paul's ask): **recommendation = NO overlay file.** Keep ONE divergence mechanism, the `staging` branch: commit the QA-only value straight into `instance/fernwood.json` on staging with the rebuilt viewer beside it, and guard the KEY with a fixture register (`tools/qa-fixtures.json` + `check-qa-fixtures.py --check`, red when a registered qaValue is present at a ref destined for main — wired into build-viewer.yml on main, the pre-push hook for main/prod refs, and session start; `qa-divergence.py` prints DECLARED FIXTURES). Why not an overlay: every layering option forces a deploy-time viewer build, which hollows out `check-live.py` and `build-viewer.py --check` on staging (two green checks over an artifact nothing serves). Rule stated: a value that will one day be true at Fernwood is a STAGED prod change (the A+ default, identity.theme); only a value that must never be true there is a fixture. **Paul's call pending** — he had leaned overlay ("sounds good overall") and asked for the analysis; the register + check are built either way and foreclose nothing. Migration procedure: set registered keys to prodValue on main, rebuild, commit — never merge viewer.html.
- stage-note: ⭐ DESIGN NOTE for 4b/5b/6b [paul-stated 2026-09-04 ~6:40 AM ET, ruling the honesty string]: *"the login you should have — I think your username and the password you have to input, and it can remember your username if you click the box or something like that."* Paul's mental model of the door is a REMEMBERED USERNAME + a TYPED PASSWORD, not a minted token pasted once. The credential mechanism (3a: sha256 of what is presented; a word she would tell Paul is allowed by Q5) can serve that surface — the presented string can BE a password, and the personId the remembered half — but the door's surface must be designed to his model, and the ux seat reads it at 5b. Nothing here changes the KV row.
- stage-note: 6a SHIPPED 2026-09-04 ~2:05 AM ET — QA first (commit "C6 6a: dual-accept on the read paths"), then PROD by the cherry-pick path: a `prod` branch cut from origin/main took the five WORKER-class commits (I1/I2 guard, Guru 4a, 4b, the probe grader, 6a) with `git diff origin/main prod -- viewer.html instance/ engine/` EMPTY, deployed by wrangler and pushed to main (3c71b60). Gate: `authOk(master) || (grant && hostAgrees)`; administrator = the master's reach; member = `/api/metrics` POST + `/api/vault/*`, shaped 403 otherwise (the seat's code, if it names one, replaces one line); metrics batches carry `via`. Proven on QA with three fixture grants (admin · member · other-estate): no credential 401 · master 200 · admin 200 · member 403 on feedback · member 200 on metrics POST with the batch stamped `via: grant` · member 403 on promote-species · other-estate 404. Prod after deploy: /health ok · ambient 288 rows · no-credential 401 · master 200 · classify 200. `read-mom-engagement --pickup` reads 0 sessions since lap 8 — that is her recent use, not the deploy. `test-feedback-cycle.py --live` is Paul's (a prod write) and was NOT run. 6b/6c wait on the visit.
