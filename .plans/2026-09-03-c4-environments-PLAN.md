...
- stage: build
- stage-note: **QA ORIGIN BEHIND CLOUDFLARE ACCESS 2026-09-04 ~8:15 AM ET** [paul-stated: "I've not shared access with anyone, so let's just keep building quickly — I'm good on you adding access"; driven in Chrome at his ask]. Two Access applications: the Pages-made *fernwood-qa - Cloudflare Pages* (preview wildcard, policy Allow Members) and a self-hosted *fernwood-qa production* on `fernwood-qa.pages.dev` with policies **paul** (Allow · Emails = his) and **qa-walk** (Service Auth · any Access service token — only `qa-walk` exists). Zero Trust needed a plan chosen first (Paul, Free). The service token was generated by Paul and never crossed the agent's screen (form filled to the button; values via a mode-600 hand-off, verified BY USE: 200 with the headers, 302 to login without; the hand-off deleted). Token lives in `.private/cf-access-service-token.json` (mode 600, gitignored); `tools/qa_access.py` adds the two headers whenever a tool fetches the QA page (qa-walk · check-text-size-default · uniqueness-ledger · check-live); the Worker is untouched (its own token gate). ⚠️ CI has NO token: `qa-build.json` polls and any CI fetch of the QA page now meet the login — the deploy itself is unaffected (Pages, not a fetch). CF_ACCESS_CLIENT_ID/SECRET as repository secrets are Paul's to set if CI ever needs the page. Proven after: qa-walk clean at 414×A+ through Access; text-size default lg; check-live green.
- stage-note: 2b PER-ESTATE HYGIENE BUILT ON QA 2026-09-04 ~12:45 PM ET — SURFACE commit fa51999 "C4 2b: the question queue's four keys are per-estate — ESTATE_ID reaches the viewer at build from instance.json `estateId`, estateKey() segments them, a one-time carry-over copies the old key forward, check-storage-keys fails on a bare use" (detail in the 2b step below)
...
**2b · Pre-domain hygiene** — ✅ **PER-ESTATE HYGIENE BUILT ON QA 2026-09-04 ~12:45 PM ET** (SURFACE commit "C4 2b: the question queue's four keys are per-estate — ESTATE_ID reaches the viewer at build from instance.json `estateId`, estateKey() segments them, a one-time carry-over copies the old key forward, check-storage-keys fails on a bare use"): `{{ESTATE:id}}` placeholder (build FAILS LOUD without `estateId`; Fernwood `est-3c9f1a` = the prod Worker's; the condo paper model carries a placeholder id until its grant mints one), `STORAGE_KEYS_PER_ESTATE` roster + `estateKey()`, the four `momQueue.*` consts wrapped, selftests 4/4 + builder 8/8; condo builds with its own id. ⚠️ The QA Worker keys KV under `est-qa0001` while the viewer's storage segment is the INSTANCE's id — two ids on QA by design (the Worker's is the environment's fixture estate); the onboarding plan makes the grant the source of both. 2c's pre-staging still open. *(history:)* 🟡 ROSTER + GUARD DONE 2026-09-03; the `momQueue.*` per-estate hygiene and 2c's pre-staging still open — `STORAGE_KEYS` declared in `viewer.html` (18 keys, matching the measured count; declarative — usage sites keep their literals per 4f); `tools/check-storage-keys.py` scans for `"tateTracker.` literals outside the roster, exit 1 on an unrostered key, exit 2 (fails CLOSED) on a missing roster; selftest plants a 19th key and it fails. Added to `CLAUDE.md`'s session-start block so it has a caller. Root-serving proven by 3d green against QA. · agent · reversible · a custom domain (like Pages QA) serves at `/`, not
