...
- stage: build
- wip-exception: measured 2026-09-10 — this item has been BUILT and on staging since 2026-09-04 (stage-notes below); the header lagged its own log and the ladder read it as "ready, nothing owed". Not a new pull into the band.
- stage-note: 2026-09-10, later — ⏸ **SET ASIDE `[paul-ruled]`: "let's not worry about this. Mom is gonna stand up her own condo at her own time. She'll mint her own estate and all that so let's not create anything artificial. If we wanna look at a condo model now let's look at my Grant Park condo — that's something we can really build out together by me using [it]."** Not built further; the row carries the consequences and one flag (is the `paul` deployment this same condo?).
- stage-note: 2026-09-10 — header corrected by the backlog-refinement window from the plan's own stage-notes: 1b/1c/2a–2d ✅ 09-03, the 2e read fixes SHIPPED TO STAGING 09-04, place claims 53 → 10. The `paul` deployment (`est-d93508`, Worker `myhome-paul`) is this condo and Paul is on it. Open, all Paul's: Q4 (AI-boundary third path, ai-advisor) · Q5 (Midtown elevation) · Q6 (the estate id) · Q9 first half (the Guru prompt's owner); then a retro. Not shipped through gate ①; the paul deployment serves ebdf172 (09-06) per the coordinator, unverified here.
...
- stage-note: **2e — PAUL'S FIRST READ, 2026-09-04 ~5:40 AM ET** [paul-stated, verbatim-ish]: (1) *"figure out the jump strip versus the summary menu"* — three layers say the same thing (a jump-strip emoji, a summary tile with a line, the big card); rationalize them. (2) *"the summary menu looks weird right now — truly just gaps"* when modules are off: an OFF module needs an AESTHETIC adjustment so the grid reads intentional, not holed; *"a lot of these can be settings — turn on or off."* (3) *"a standard view of what we really need to populate manually versus what's automated, so we don't miss something"* — the map is blank at the condo, references too. (4) ⭐ **THE RULE: *"it's better to not display something rather than display something that's empty."*** → generalizes the 9/03 ruling (no jump-strip tile, no empty Mama's Perspective) to EVERY container: an empty map, an empty references card, an empty anything is hidden, never rendered hollow. Actions: BACKLOG rows C7-R1 (strip vs tiles vs cards), C7-R2 (off-module grid aesthetics + on/off settings), C7-R3 (the manual-vs-automated population view — INSTANCE-RECIPE.md is the seed), and the rule becomes a CHECK: the walk asserts no rendered container is empty (`qa-walk.py` / the falsifier), red at the condo today for the map and references.
- stage-note: **2e — second read, ~5:55 AM ET** [paul-stated]: (5) the header's *"Early September · Friday, September 4"* is redundant — *"just say the date"* → engine change (C7-R4; rides the migration, Fernwood too). (6) the condo tagline *"A field journal for a home in the city"* — *"I don't know how much that makes sense"* → placeholder, put to the content steward. (7) *"Atlanta, Georgia · 1,050 ft in Midtown Atlanta"* repeats the city → FIXED in the condo instance (suffix "in Midtown"). (8) the condo rendered at A, not A+ — *"be sure we're using the text size Mom sees"* → FIXED: the condo declares `lg`; the "normal" fixture value is retired from every real instance (the per-instance switch is exercised only by the build selftest's temp fixture). ⚠️ The empty-container walk passed the condo CLEAN while Paul saw a blank map and an empty references card: both live in cards COLLAPSED at load, and the detector measured only what was open — it must expand each card before measuring.
- stage-note: 2e FIXES SHIPPED TO STAGING 2026-09-04 ~6:20 AM ET, commit "Condo read: the header says the date once, no basemap → no map section, events rostered and declared absent at the condo" (SURFACE — Fernwood's header changes too, so it rides the migration): (a) `renderPropertyMap` returns nothing when the zones record declares no basemap — the engine's typed FALLBACK_* paths are Fernwood's, a derivation leak noted for retirement; (b) `events.json` was inlined UNROSTERED, so every build carried Pickens-County events — rostered in `check-data-inline.py` with a declared-absent shape, the condo declares it absent (0 mentions of Pickens in its build); (c) the header prints the date once; (d) the walk expands every card before judging emptiness and treats a broken `<img>` as no media — it still cannot see a map that is a *stage with buttons*, which is why (a) is a renderer rule, not a detector. Condo walk clean; `check-data-inline` in sync; `--selftest` holds. **Paul's read items still open:** C7-R1 strip vs tiles vs cards · C7-R2 off-module grid aesthetics + on/off settings · C7-R3 the manual-vs-automated population view · C7-R5 "Coming up nearby" placement (fresh data near the top) · the condo tagline (steward).
- stage-note: commit "Place claims moved: plant-context callouts into the record, the meteor verdict derived from the sky record" (SURFACE, 2026-09-04 ~7:30 AM ET) — see BACKLOG uniqueness ④ MOVED; condo place claims 53 → 10; candidates.json and events.json rostered (both had ridden unrostered into every build).
- stage-note: commit "Place claims: the last ten — eight rewordings, two dark-sky tips removed, the station legend gated" (SURFACE, 2026-09-04 ~8:50 AM ET) [paul-stated "go for it"]: the steward's rewordings applied (the home, not the house; the machines we keep; the on-site gauge; the regional grid), the section title renders the estate's name, the capture button names the journal, the two hardcoded dark-sky sentences are gone from the meteor tips, and the station legend row renders only where the estate declares a station. Re-measured: shared sentences 165 → 143; **place claims rendering at the condo 10 → 0**; the ratchet holds at 0 from here. Fernwood build: 15 cards, no script errors; condo walk clean.
