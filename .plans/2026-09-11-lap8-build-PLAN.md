...
- stage-note: 2026-09-11 (after Paul's lap-7 walk) — **LAP 8 HOLDS until the testing-revamp plan is clear; ROW T lands WHOLE and FIRST** `[paul-ruled, revamp window: "if we have to hold lap eight until all this is determined and we have a clear plan, that's fine… I'd rather not split it up unless there's a really good reason — not just time and effort"]`. Supersedes the 9:10 AM lap-9 stage-note. Lap 8 does not open until Paul has read `.plans/2026-09-11-testing-revamp-PLAN.md`; its beat-6 table leads with T; the door's battery runs on the new unit. A split needs a STRUCTURAL reason named by a seat (a ruling not given · a dependency on lap 8's own rows · a falsifier that cannot run before the door exists). (coordination)
- stage-note: 2026-09-11 — **RULED at Paul's lap-7 walk: "the account is always the first layer."** Row A's door is the ACCOUNT layer (sign in · create an account); *set up a place* moves INSIDE to the empty shelf after the account exists; a just-signed-out device leads with sign-in (a rostered post-sign-out marker so B13's lede renders). D7/B2 founding-first on the bare door is superseded on this point. §10's ux-expert closure at open designs the door and the shelf under this rule. (coordination)
- stage-note: 2026-09-11 ~9:10 AM ET — **ROW T WITHDRAWN FROM LAP 8 → LAP 9** `[paul-ruled: "let's keep that as it was planned in terms of the commitment, and then we'll move the testing lap implementation to lap nine"]`. Lap 8's commitment is as ruled at ~12:10 AM (A · C · E · F · G · riders; B its own gate); the testing revamp window AUDITS lap 7 and MONITORS lap 8's battery, then row T builds in lap 9 beside the weather card. The 7:00 AM stage-note above is superseded. (coordination)
- stage-note: 2026-09-11 ~7:00 AM ET — **NEW ROW T · the testing architecture** `[paul-ruled: "move it up to the next lap… do it in one piece"]`, ahead of A in build order: gate ① unit → (journey, lens) · the lens/journey split in journey-walk · the per-lap cell list and the longest-unwalked print · an impact-scoped re-run rule (change classifier) · per-run unspent invite for J1. Source: `.plans/2026-09-10-testing-architecture-PLAN.md` (now `ready: paul-ruled`) and CYCLE-LOG "LAP 8 GAINS A ROW". The re-audit at open sizes it by symbol and says whether one piece fits beside the door; fallback ruled: two laps. (coordination)
- stage-note: 2026-09-11 ~3:35 AM ET — two security findings from the ask-design plan join row A's feedback-path work `[paul-ruled: "folded in with lap eight"]`: handleFeedback writes deviceId beside a personId (verify the site, then write null when a personId is present — the recovery record's field rule applied to the existing writer), and GET /api/feedback + the zones read use dateKey(scopeOf(env)) not scopeFor(request) (TIER 1 · 54) — both by symbol in the A2 conversion at the re-audit. (coordination)
