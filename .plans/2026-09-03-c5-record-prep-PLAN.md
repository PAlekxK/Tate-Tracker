...
**6c · The two `list({prefix})` calls + `zones:all`** — 🟡 **BUILT + QA-PROVEN 2026-09-03; prod on Paul's push** — `listBothEras()` unions `<estate>:<kind>:` and `<kind>:` for conversations and `zones-last-seen`; `zones:all` + `observations` read/write the prefixed key and are COPIED at cutover (wrangler, remote, byte-for-byte checked). Positive controls on QA (their failure is silence): a planted prefixed conversation → `/api/conversations` **counts 1**; prod's `zones:all` planted under the prefixed key → QA `/api/zones` returns **18 zones**, and the read-back is **IDENTICAL** to the source bytes (`cmp`); `GET /api/zones?d=<harness>` wrote `est-3c9f1a:zones-last-seen:<harness>` and `/api/zones-sync-status` lists it with the canon version. ⚠️ Measured: KV is eventually consistent — a key put via wrangler read as absent for ~30 s from the Worker; a probe that reads immediately after a put reports a false miss. — agent · reversible **while the unprefixed keys are not deleted**
