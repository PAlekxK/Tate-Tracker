...
- stage-note: 2026-09-12 evening — ⛓ **THE DEPENDENCY CHAIN IS NOW THE PRIORITY ORDER** `[paul-ruled 2026-09-12: "let's adopt this chain as our priority"]` (`BACKLOG.md` § ⛓, `3a206ba1`). **This plan is LAYER 3, sitting on an unbuilt LAYER 1** — the per-estate canon STORE (TIER 2 · 12, still `concept`). ⛔ **So the entry path cannot FINISH above the root:** `handlePromoteSpecies` 503s permanently at every household (TIER 1 · 71), so a confirmed entry has nowhere to land until layer 1 exists. ⭐ **This does not refute the design** — the reshaped step 1 (read prior state from KV; git a write-only mirror only where a token exists) is the shape layer 1 needs, and §D's georeference finding is load-bearing for it. **It bounds what a build may CLAIM to deliver, not whether the design is right.**
  ⭐⭐ **AND IT GIVES R3 A SECOND, INDEPENDENT REASON TO HOLD — which is why it is recorded here rather than left in the chain.** R3 holds the build window until the `design` band clears; the chain holds it until layer 1 is built. **Two unrelated gates that happen to agree.** ⛔ Either one alone could be argued away in a hurry; together they are not a scheduling opinion. *(Verified here, not inherited: `canonFor(env, scope)` / `foreignCanon()` guard `handleTodayLine:2114`, `handleClassify:2187`, `handleChat:2966` and `handlePromoteSpecies:3477`, plus a throw at `:2538` — **the journal's asking and a module's adding fail at the same line**.)*
  ⚠️ **Cross-reference, filed by the backlog window and re-measured here — it is §E's row-78 class again:** `handleSuggestSpecies` (`worker.js:3338`–`:3446`, serving `/api/pending-species`) holds **zero** git references and writes KV twice, **so the STAGING half of promotion already works at a household today** — a person can submit a photo and have the suggestion stored in their own estate. It stops only at promotion. ⛔ But it keys on **`scopeOf(env)` three times and `scopeFor(request)` never**, so the day one deployment holds two households, **one person's suggestion lands in the other's queue.** Latent, not live — and armed by exactly the multi-household work this chain sequences.
