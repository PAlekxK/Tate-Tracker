...
- stage-note: Q6 the honesty strings — RULED 2026-09-04 ~4:50 AM ET [paul-stated]: one place, the reader's word for the corpus ("the Almanac" at Fernwood; each estate names its own), the vault's reader word is **the safe** ("in the safe — that part of the Almanac needs the login before it can be read"). Content-steward review applied the same hour (`.content-reviews/2026-09-04-guru-honesty-strings.md`): six strings in `LOOKUP_STRINGS_TEMPLATE` (worker.js) — NOT_IN_RECORD · NONE_RECORDED (an empty domain, not a missing thing) · AMBIGUOUS (a question, not an imperative) · LOGIN_REQUIRED · NO_SOURCE ("not in the library — the part of {journal} that…", never the corpus-wide phrase on one room's search) · NO_LIBRARY (the stray "index is not loaded at this estate" retired — it put *estate* in front of a reader). `{journal}` = the instance's short word with its own article, from digest core.identity (journalShort or derived); `no such tool` is an `error`, never a `reason`. VOCABULARY §3b gained **the safe** (surface; `vault` stays schema) and **the library** (not *shelf*: vehicles.json already uses on-shelf). Live: both honesty rows GREEN after the prompt was told to relay the reason in its own words (it had paraphrased "locked behind your credentials"). **RULED 2026-09-04 ~6:40 AM ET [paul-stated]: "login"** — the sentence stays "needs the login before it can be read". Q6 is CLOSED: all six strings are Paul-approved via the steward's text.
- stage-note: COST ANALYSIS DELIVERED 2026-09-04 ~1:15 PM ET — `.engineering/2026-09-04-guru-cost-analysis.md`: on QA's log a cold turn costs $0.19 on the digest path vs $0.04 on the core path (5.0× less), per-turn $0.045 vs $0.012; on prod 3 of 7 turns in 14 days were cold and were most of the bill — so YES, the lookups cut cost exactly at the first question; keep the 5-minute TTL (her sessions are hours apart, a 1-hour TTL would only double the write). The Worker records usage, not dollars — cost is computed, not logged.
