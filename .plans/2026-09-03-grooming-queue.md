...
|---|---|---|---|---|---|---|---|---|
| 0 | Environments + repo structure + rename | C4 | O3 | engine · must-not-diverge | ran: practice-steward · engineering-partner | ai-advisor · ux-expert · content-steward (no model, nothing she sees, no copy) | — | **plan DRAFTED** `.plans/2026-09-03-c4-environments-PLAN.md` · check silent · **8 questions open before the stamp** |
| 1 | Record prep, data-model steps 1–5 | C5 | O3 | engine · must-not-diverge | engineering-partner (schema, KV prefix, manifest, config derivation) · practice-steward (the engine-manifest CHECK is a check) · content-steward, narrow (the product name in her prose) · (cite: data-model design §3–§5, §8) | ai-advisor (no model on the path) · ux-expert (nothing she sees changes) · user-researcher (no user question) | C4 (prefix + manifest only — steps 2, 3, 5a ship independently) | **engineering seat DONE** `.engineering/2026-09-03-c5-record-prep.md` · practice-steward running · content-steward's narrow question is now upstream in the naming seats |
