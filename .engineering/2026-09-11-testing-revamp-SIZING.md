...
2. ⭐ **`check-release-docs.py` will go RED between T5 landing and Paul making this edit, and that is the checker working.** It compares the beats CYCLE-MAP declares against the beats `release-state.py` publishes. **⛔ Flags, never edits — which of the two is right is a judgement, and it has gone both ways** (CLAUDE.md's own line). Do not quiet it by editing the map.
...
| **T6 the CYCLE-MAP edit** — a release-condition change is Paul's | does the tool need the map edited first? | ⛔ **NO SPLIT.** The gate can carry the new unit before the map's wording catches up; `check-release-docs.py` goes red in between, **which is correct and must not be quieted**. The edit ratifies what landed |
