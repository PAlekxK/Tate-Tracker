...
- [x] ✅ **ABS sheet stock — OWNED, `paul-stated 2026-09-08`** *(date/thickness not captured; this line previously read "no receipt found")*. ⛔ **The same line used to claim "the mesh-weld path below is fully stocked" — that was FALSE and is withdrawn 2026-09-08:** the mesh weld needs a broad flat tip and no iron Paul owns has one (see the corrected tool line below). It is the BONDED-PATCH path that is fully stocked. — 1/16" (0.06") for contoured backing patches, plus a little 1/8" for rebuilding clip mounts. Black is fine (it's all hidden). [Cut-to-size at TAP Plastics](https://www.tapplastics.com/product/plastics/cut_to_size_plastic/abs_sheets/524) (small pieces, both thicknesses — verified in stock; don't buy a 4×8 sheet). *(Even if the panel turns out to be PP, ABS sheet is still the backing material — you just change the adhesive.)*
...
- [x] ✅ **Plastic welder / hot-stapler — OWNED: ATOLS welder + 1000 staples, pure copper transformer, acquired 2026-06-22.** He bought the exact unit this line recommends. Plus **YehVeh welding tips ×2 (2026-06-22)** for the iron. Original research kept for reference: *(for the mesh-weld method + rebuilding clip tabs — see Step 2)* — [ATOLS hot-stapler kit](https://www.amazon.com/ATOLS-Plastic-Welding-Machine-Stapler/dp/B091T368MK) (~$20, verified) or [Harbor Freight 70770](https://www.harborfreight.com/hot-stapler-kit-70770.html) (~$30, verified). A plain soldering iron also works for melting mesh in. ⛔ **WITHDRAWN `2026-09-08` — the 9/06 election was wrong.** It read: *"the Weller WLSKD7012A 70 W station with its 2.4 mm chisel"*. **No Weller tip is FLAT** — WLT tops out at a 2.4 mm screwdriver, MT at a small chisel, and both are point tools. Melting mesh in is an **area** job needing a broad heated foot. `paul-observed 2026-09-08`; the YehVeh tips (16 mm + 20 mm flats) are the right shape and fit **neither** Weller — trial-fitted on both, no fit. **Also note this checklist line conflates two different methods:** the hot-stapler does STAPLES, not mesh — it heats the staple by resistance and has no hot surface to press a screen with. ⭐ **The mesh weld needs an 80 W plastic-welder pencil with a ~5.2 mm SLIP-OVER rod (⚠️ not a 1/4"-28 threaded one — JOUNJIP and Gino are threaded), or a kit that ships its own flat tips.** ⛔ **The ATOLS escape hatch is CLOSED — `verified 2026-09-08`:** its manufacturer package list is *welder · pliers · utility knife · carrying case · 1000 staples*, and no ATOLS variant ships a flat tip (variants are colors). An iron is the only route to the mesh weld. Full tip-compatibility table + the three reinforcement paths: `.private/service-records/TOOLS.md` § Soldering irons and § Plastic repair. ⚠️ **Before buying ANY replacement tip, run the amended two-question caliper check (`2026-09-08`): is the shank THREADED or SMOOTH, and then what diameter?** A reviewer who measured reports Weller's MT shank is a `#10-24` machine screw rather than the plain set-screw rod this record assumed — unverified, but it is the same mount ambiguity that has already produced two dead tip orders. Also ⛔ **ignore Amazon's "newer model" pointer from the MT1 page to `WLTC08IR30` — that is for the WLIR30 iron, which Paul does not own; the station's pencil is WLIR70.**
