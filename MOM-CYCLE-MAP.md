...
| `tools/check-estate-neutral.py` | does a household surface name **another household**? | 2026-09-07: Fernwood's gauge record rendered at households in Roswell, Dahlonega and Bangor — and this read ✅ 311 needles / rendered=0 against the very origin four seats walked. ⚠️ It tests for **NAMES**; that leak was numbers and possessive pronouns. Its bare form does not scan `viewer.html` at all — use `--url` or `--page` |
| `tools/check-household-isolation.py` | can two households share ONE namespace without seeing each other? | the tenancy question asked of the store rather than the screen — the sibling of the check above, one layer down |
| `tools/read-geocodes.py` | did households get **placed**, and when not, WHY? | built 2026-09-07 to repair a defect: the geocode was instrumented the same afternoon and `grep -n geocode tools/*.py` returned **zero** — a record nothing read. Outcomes only (`placed` · `refused:box` · `failed:*`), never an address, never coordinates |
| `tools/read-onboarding.py` | what did people say while setting up — and **what's missing**? | the only line where someone can name a need nobody anticipated. Reports real · synthetic · unknown, and which run is authoritative |
...
| `tools/check-vocabulary.py` | **is `VOCABULARY.md` still TRUE of the schema?** Five checks — a rejected term used as an identifier, a canonical term used nowhere, a double-booked key, canon contradicting itself, a taken name minted anew | Built 2026-09-02 `[paul-stated: "make it deterministic where possible to the data schema"]`. ⚠️ **Scope is the N8 guard, not timidity:** `property` appears **433×** in `viewer.html`, so a checker over prose would be red forever — the *costly control* signature. It governs **schema-bearing surfaces only**; legacy prose is counted, never graded. ⛔ **It caught its own defect on its first live run** — flagging the sentence *"the key is `estateId`, never `propertyId`"*, i.e. reading a rule AGAINST a term as a use of it. In Markdown a backticked span is a mention, not a use. Selftest 10/10 |
| `tools/check-storage-keys.py` | **is every browser-storage key ROSTERED?** Scans `viewer.html` for `"tateTracker.` literals outside the `STORAGE_KEYS` declaration and fails on an unrostered one; fails CLOSED on a missing roster | storage is per ORIGIN — the custom-domain move (C4 2d) strands every key on her phone that the one-time migration does not know about. 18 keys rostered 2026-09-03 (C4 2b); a 19th appearing silently is the failure it exists to catch |
| `tools/check-engine-manifest.py` | **is every tracked file CLASSIFIED (engine · config · instance · mixed · private-pointer), and has any classification rotted?** P1/P2 fail; P3 `skipped` until an engine remote exists; P4/P5 counted and self-arming | the engine split (C4 5a–5d, C5) cannot start from a tree where nobody can say which files are shared — `ENGINE-MANIFEST.md` derives class from layout + the three existing rosters, and this is the coverage proof. Selftest proves every predicate by mutation, incl. THROWING on a 404 page |
| `tools/build-viewer.py --check` | **is `viewer.html` BYTE-IDENTICAL to the engine template + the instance's canon?** Rebuilds to a temp path and compares; prints the first differing offset | C4 5b: the viewer is now BUILT from `engine/viewer.template.html` + `instance/fernwood.json` + the 12 rostered JSONs (roster read from `check-data-inline.py`, never restated). Red is the check working: either canon moved (rebuild) or one of the four writers that still edit `viewer.html` directly ran (`--extract` absorbs it). Selftest 8/8 incl. a declared absence building an empty const and a 404 template THROWING |
| `tools/check-condo-falsifier.py` | **can the engine render a PLANTLESS estate without a fork?** Full mode REFUSES (exit 2) until C5 3a exists; `--pre-read` builds the condo paper model from the private sibling, asserts no engine change and greps the engine half for Fernwood identity strings | C4 5c / C7 step 2. Attempts 1–3 on 2026-09-03 found two real defects (wildlife renderers and the strip's fishing cell threw on a declared absence) and one standing finding: the engine half still names the founding instance (the un-rostered consts, `PROPERTY_DATA` first) — C5's work, at Paul's gate. Not a per-lap check; run it when engine/ or the module declaration moves |
| `tools/check-jump-strip.py` | **does the jump strip match the CARDS?** Derives both rosters from the built viewer and flags any card with no strip entry | Paul on his 2026-09-07 QA walk, on a card he could see but not reach: *"in general, the jump strips should match the cards, right?"* The strip was hand-written, the cards are generated, and nothing had ever compared them — it flags `turf`, `weeds`, `fishing`, `fieldnotes`. ⛔ It reports the MISMATCH; **which of the four earn a shortcut is a product call, not the check's** |
| `tools/check-release-docs.py` | **do the release loop's own documents still describe the loop?** Cross-reads `cycle/release/CYCLE-MAP.md`, `CYCLE-LOG.md` and the queue file for beats, tools and lap state that only one of them knows about | built 2026-09-07 during lap 3 and **found two drifts on its first run** — the map is the one artifact in this repo that had no control over it, which is the same hole `check-cycle-map.py` exists to close one loop over |
