...
- **Always offer free text — Paul steered this (2026-07-13): "don't want to limit her too much."** The optional verbatim note is present on **EVERY** confirm (not just the "Not quite" branch) — free text is Mom's *proven* channel (her input has always been conversational/own-words), so never box her into chips only. BUT keep it **invited + optional + secondary**: a quiet "add a note ›", **never a required field** (required text kills the low-attention interaction) and **never a prominent standing "leave feedback" box** — that standing-open-text-box is the exact ⭐-star/zero-usage trap the panel flagged. Her always-on "just say something" home stays the existing unified input (Save / Ask Garden Guru). Free text = AI-free verbatim capture to the ObservationStore (her words, Paul reads them).
