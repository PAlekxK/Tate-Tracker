...
- ~~**Metrics capture (added 2026-05-20)**~~ ✓ **v1 shipped 2026-05-20.** Path C from `.engineering/2026-05-20-path-metrics-capture.md`: client `MetricsCollector` IIFE buffers events to `tateTracker.metrics.v1` localStorage; flushes via Worker `POST /api/metrics` (with auth fail-soft); KV daily key `metrics:YYYY-MM-DD` mirrors the existing `cost-log:YYYY-MM-DD` shape. 7 events instrumented (session_start/end, card_expanded, conversation_started/turn/capped, field_note_saved). Privacy: structural events only; no field-note bodies or conversation content. Device-class via UA parsing. Self-exclusion via `tateTracker.metricsExclude="1"`. Analysis tool `tools/analyze-metrics.py` is queued (the data accumulates in KV either way).
- **Analysis + reporting tool (added 2026-05-20)** — `tools/analyze-fernwood.py` (or similar): reads cost-log + metrics-capture + observation-store + conversation-store from KV via the existing Worker `GET` endpoints and produces a markdown report on cost, usage, click behavior, conversation engagement, and adoption trends over a date range. The instrumentation now exists (cost-log, metrics, observations, conversations all persist to KV); the reporting layer is the next move so Paul can actually *see* the engagement signal the rubric promised. Should be a Python script + Claude-in-chat synthesis, per the engineering-principle "analysis lives in tools/, not a dashboard." Mirrors `tools/record-daily-rollup.mjs` shape (zero-dep Node/Python, runs locally, posts to no external service). Scope: one script that takes `--start YYYY-MM-DD --end YYYY-MM-DD` and renders a markdown report Paul reads (or feeds to Claude for richer analysis). Path-eval before code.
- **Unified-input UX redesign (Q10, added 2026-05-20)** — path-evals complete + **design locked 2026-05-20 evening after live E2E test.** See [[project_fernwood_almanac_save_model]] for the architecture decision. Path-evals: `Tate-Tracker/.ux-reviews/2026-05-20-unified-input-redesign.json` (ux-expert) + `Tate-Tracker/.engineering/2026-05-20-path-unified-input.md` (engineering-partner). **Design lock:** single textbox + two action buttons ("Save" no-AI quick-log / "Ask Garden Guru" AI conversation); **auto-save everything to almanac on action** (no opt-in disclaimer); per-entry "this matters" star/pin affordance for filtering; whole conversation (user turns + Garden Guru replies) saves on completion. ux-expert's collapsed-state journal-pull + symmetric button weight + seeded prompts recommendations preserved. engineering-partner's Path C (Hybrid, behind feature flag, metrics-baseline-first) preserved. Implementation queued for future session. **Open naming question:** the saved-entries view currently called "Field Notes" probably wants to be renamed "Almanac" to match the form-layer frame; content-steward call when implementing.
