...
6. **Rotate the GitHub fine-grained PAT used 2026-05-13.** The token starting `github_pat_11A5K…` was pasted into a Claude Code session to push the workflow file, so it lives in that conversation history and should be considered compromised. Go to https://github.com/settings/tokens?type=beta → click the token → **Regenerate token** (keeps the same name + scopes, just rotates the value) or **Delete** if no longer needed. Save the new value in a password manager. Required permissions if regenerating: Contents Read+Write, Workflows Read+Write, Metadata Read (mandatory), Tate-Tracker repo selected. Adding Actions Read+Write is optional but would enable API-triggered manual workflow runs.
