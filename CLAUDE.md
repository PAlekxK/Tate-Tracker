...
- **Gmail mbox receipts → fold into fleet records.** Full Gmail export (`~/Downloads/All mail Including Spam and Trash-002.mbox`, complete ~14K-msg history — the big Takeout zips are Photos/Drive, NOT mail) mined deterministically for vehicle receipts. **36 clean candidates** saved (gitignored) at `.private/service-records/EMAIL-RECEIPTS.md` — Bronco eBay haul (tailgate latch/weatherstrip/window switch — net-new vs Amazon), GTI (Autohaus water-pump invoice, **Tire Rack tires $774.97 Oct-2022 = a service event NOT on the card**, Express Oil invoices), DR200 OEM handlebars, DR-Z refresh parts, first Tiguan part (fender flare). **NEXT: de-dupe vs `AMAZON-PARTS.md` → fold (private detail + public summaries) → Paul reviews 2 big PayPal payments ($1,732 Sep-2024 / $1,000 Jun-2025: vehicle or not?) + install status.** Reusable parser pattern proven.
