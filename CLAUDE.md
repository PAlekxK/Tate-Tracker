...
- Phase F image input — **benched 2026-05-20** per design conversation; but the Garden Guru interview surfaced this as the killer use case for both Paul + Mom, so the bench is worth re-examining post-rubric
- Mt. Cuba reference / native-cultivar trials — captured in `plants-to-consider.md`; revisit when planning new plantings
- **Metrics capture (added 2026-05-20)** — instrument actual usage so we can see if Mom is using the dashboard / Garden Guru. The Q11 success criterion ("Mom opens the dashboard regularly") and the Q4 wedge claim ("the personal library is the moat") both depend on observed engagement data, which we don't currently collect. Engineering scope: lightweight pageview + interaction logging (likely client → existing Worker → KV), privacy-respecting (family-only). No third-party analytics. Path-eval pending.
- **Unified-input UX redesign (Q10, added 2026-05-20)** — one minimalist textbox beneath the 6 main cards, two buttons ("Record an observation" → capture, "Ask Garden Guru" → ask). Replaces the two-card layout (Field Notes + Garden Guru) shipped in Phases B/C/D/E. ux-expert + engineering-partner path-eval pending.
...
| Phase | Scope | Status |
|---|---|---|
| **D — Capture UX rebuild** | Replace the Field Notes modal with a single always-visible text box + mic at the top of the card. Drop the category and species pickers from the user-facing form entirely. Timestamp captured automatically. A `POST /api/classify` Claude call assigns category/speciesId behind the scenes after save. | ✓ Shipped 2026-05-19 (commit 783e72c). Worker `/api/classify` endpoint live; inline composer + async classify wired; fuzzy species matching against curated *_DATA. **Pivot 2026-05-20:** capture path becomes pure-text log per the no-AI-on-capture design principle Paul articulated this session ([[feedback_no_ai_on_capture]]). Classify-on-save call slated for removal from `viewer.html`; Worker `/api/classify` endpoint kept dormant for possible reuse in the future batch roll-up (Phase G). Code removal pending; doc reflects decision. |
